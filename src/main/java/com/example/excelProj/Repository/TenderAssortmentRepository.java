...
public interface TenderAssortmentRepository extends JpaRepository<TenderAssortments,Long> {
    ...
    @Query(value = "select * from jobportal.tender_assortments where employer_id=:id && notification_for='employer' ORDER BY tender_assortments.notificationDate DESC",nativeQuery = true)
    List<TenderAssortments> getAcceptOrDeclineNotification(@Param("id") Long id);

    @Modifying
    @Transactional
    @Query(value = "update tender_assortments set is_applied=:isApplied AND notification_from='recruiter' AND notificaton_for='employer' where (tender_id=:tenderId AND recruiter_id=:recruiterUserId AND employer_id=:employerUserId)")
